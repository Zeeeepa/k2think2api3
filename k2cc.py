...
def create_environment_config(repo_dir, api_key, email):
    """Create environment configuration file"""
    config_file = os.path.expanduser('~/.k2think_config')
    
    config_content = f"""# K2Think + Claude Code Configuration
# Auto-generated by k2cc.py on {time.strftime('%Y-%m-%d %H:%M:%S')}
...
alias k2-stop='pkill -f "python.*main.py"'
alias k2-status='ccr status && curl -s http://localhost:7000/admin/tokens/stats | python3 -m json.tool'
alias k2-logs='tail -f {repo_dir}/server.log'
alias ccr-dangerous='ccr --dangerously-run'
alias ccr-dev='DANGEROUSLY_RUN_IN_DEV=true ccr code'

# Helper Functions
k2check() {{
    echo "Checking K2Think Stack..."
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    
    # K2Think Server
    if curl -s http://localhost:7000/ > /dev/null 2>&1; then
        echo "✅ K2Think Server (7000): Running"
    else
        echo "❌ K2Think Server (7000): Not responding"
    fi
    
    # Claude Code Router
    if curl -s http://127.0.0.1:3456/health > /dev/null 2>&1; then
        echo "✅ Claude Code Router (3456): Running"
    else
        echo "❌ Claude Code Router (3456): Not responding"
    fi
...
def update_bashrc(config_file):
    """Update bashrc to source K2Think configuration"""
    bashrc_path = os.path.expanduser('~/.bashrc')
    
    # Backup bashrc
    if os.path.exists(bashrc_path):
        backup_path = f"{bashrc_path}.k2think.backup"
    ...
    if os.path.exists(bashrc_path):
        with open(bashrc_path, 'r') as f:
            content = f.read()
            if config_file in content or 'k2think_config' in content:
...
def create_startup_guide(repo_dir, api_key):
    """Create comprehensive startup guide"""
    guide_path = os.path.join(repo_dir, 'STARTUP_GUIDE.md')
    
    guide_content = f"""# K2Think + Claude Code Startup Guide
...
# The router is automatically configured for development mode
```
...
All these are automatically set when you reload your shell:

```bash
K2THINK_HOME={repo_dir}
K2THINK_SERVER=http://localhost:7000
K2THINK_ROUTER=http://127.0.0.1:3456
K2THINK_API_KEY={api_key}
DANGEROUSLY_RUN_IN_DEV=true
...
k2-restart        # Restart entire stack
...
### Claude Code
```bash
ccr-dev           # Start Claude Code with dev mode
...
echo "Write a Python function to sort a list" | ccr code
```

### Development Mode Features

The router is configured with:
- ✅ Development mode enabled (`DANGEROUSLY_RUN_IN_DEV=true`)
...
```
K2Think Installation: {repo_dir}
Configuration File:   ~/.k2think_config
Bashrc:              ~/.bashrc
Router Config:       ~/.claude-code-router/config.json
Router Logs:         ~/.claude-code-router/logs/
Server Logs:         {repo_dir}/server.log
Tokens:              {repo_dir}/data/tokens.txt
Credentials:         {repo_dir}/accounts.txt
```
...
2. **Use aliases for quick access**
   ```bash
   k2-start && ccr start  # Start everything
   ```

3. **Monitor logs when debugging**
   ```bash
   k2-logs  # In one terminal
   tail -f ~/.claude-code-router/logs/service.log  # In another
   ```

4. **Restart if things get stuck**
   ```bash
   k2restart
   ```
...
5. **Keep tokens fresh**
   ...
   - Re-run token extraction if API fails

## 🌟 What's Next?

Now you can:
- ✅ Use Claude Code powered by K2Think
- ✅ Build applications with AI assistance  
- ✅ Access K2Think's reasoning capabilities
- ✅ Work entirely locally without API costs

**Happy Coding with K2Think! 🚀**
"""
...
def print_setup_summary(repo_dir, api_key, config_file, guide_path):
    """Print comprehensive setup summary"""
    print()
    print(f"{Colors.HEADER}{Colors.BOLD}{'='*70}{Colors.END}")
    print(f"{Colors.HEADER}{Colors.BOLD}🎊 K2Think + Claude Code Environment Configured!{Colors.END}")
    print(f"{Colors.HEADER}{Colors.BOLD}{'='*70}{Colors.END}")
    print()
    
    print(f"{Colors.CYAN}{Colors.BOLD}📁 Configuration Files Created:{Colors.END}")
    print(f"   {config_file}")
    print(f"   ~/.bashrc (updated)")
    print(f"   {guide_path}")
    print()
    
    print(f"{Colors.CYAN}{Colors.BOLD}🌍 Environment Variables Configured:{Colors.END}")
    print(f"   K2THINK_HOME={repo_dir}")
    print(f"   K2THINK_SERVER=http://localhost:7000")
    print(f"   K2THINK_ROUTER=http://127.0.0.1:3456")
    print(f"   K2THINK_API_KEY={api_key}")
    print(f"   {Colors.GREEN}DANGEROUSLY_RUN_IN_DEV=true{Colors.END}")
    print(f"   NODE_ENV=development")
    print()
    
    print(f"{Colors.CYAN}{Colors.BOLD}⚙️  Aliases Configured:{Colors.END}")
    print(f"   {Colors.GREEN}k2-start{Colors.END}         → Start K2Think server")
    print(f"   {Colors.GREEN}k2-stop{Colors.END}          → Stop K2Think server")
    print(f"   {Colors.GREEN}k2-restart{Colors.END}       → Restart entire stack")
    print(f"   {Colors.GREEN}k2check{Colors.END}          → Check service status")
    print(f"   {Colors.GREEN}k2-status{Colors.END}        → Detailed status + tokens")
    print(f"   {Colors.GREEN}k2-logs{Colors.END}          → Tail server logs")
    print(f"   {Colors.GREEN}ccr-dev{Colors.END}          → Claude Code with dev mode")
    print()
    
    print(f"{Colors.CYAN}{Colors.BOLD}🚀 Next Steps:{Colors.END}")
    print()
    print(f"   {Colors.BOLD}1. Reload your shell:{Colors.END}")
    print(f"      {Colors.GREEN}source ~/.bashrc{Colors.END}")
    print(f"      OR")
    print(f"      {Colors.GREEN}exec $SHELL{Colors.END}")
    print()
    print(f"   {Colors.BOLD}2. Verify environment loaded:{Colors.END}")
    print(f"      {Colors.GREEN}echo $K2THINK_HOME{Colors.END}")
    print(f"      {Colors.GREEN}echo $DANGEROUSLY_RUN_IN_DEV{Colors.END}")
    print()
    print(f"   {Colors.BOLD}3. Check services:{Colors.END}")
    print(f"      {Colors.GREEN}k2check{Colors.END}")
    print()
    print(f"   {Colors.BOLD}4. Start using Claude Code:{Colors.END}")
    print(f"      {Colors.GREEN}ccr code \"Write a Python function\"{Colors.END}")
    print(f"      OR")
    print(f"      {Colors.GREEN}ccr-dev{Colors.END}  # Interactive mode with dev settings")
    print()
    
    print(f"{Colors.CYAN}{Colors.BOLD}📋 Quick Reference:{Colors.END}")
    print()
    print(f"   {Colors.BOLD}Start Everything:{Colors.END}")
    print(f"      {Colors.GREEN}k2-start && ccr start{Colors.END}")
    print()
    print(f"   {Colors.BOLD}Check Status:{Colors.END}")
    print(f"      {Colors.GREEN}k2check{Colors.END}")
    print()
    print(f"   {Colors.BOLD}Restart Everything:{Colors.END}")
    print(f"      {Colors.GREEN}k2restart{Colors.END}")
    print()
    print(f"   {Colors.BOLD}View Full Guide:{Colors.END}")
    print(f"      {Colors.GREEN}cat {guide_path}{Colors.END}")
    print()
    
    print(f"{Colors.CYAN}{Colors.BOLD}⚡ Development Mode Enabled:{Colors.END}")
    print(f"   • {Colors.GREEN}DANGEROUSLY_RUN_IN_DEV=true{Colors.END}")
    print(f"   • Relaxed security for local testing")
    print(f"   • Extended API timeouts")
    print(f"   • Verbose logging enabled")
    print(f"   • Perfect for development & debugging")
    print()
    
    print(f"{Colors.HEADER}{Colors.BOLD}{'='*70}{Colors.END}")
    print(f"{Colors.GREEN}{Colors.BOLD}✅ Setup Complete! Reload your shell to begin!{Colors.END}")
    print(f"{Colors.HEADER}{Colors.BOLD}{'='*70}{Colors.END}")
    print()
...
def main():
    ...
    try:
        ...
        print()
        print(f"{Colors.CYAN}{Colors.BOLD}[11/13]{Colors.END} {Colors.BLUE}Setting Up Environment{Colors.END}")
        config_file = create_environment_config(args.dir, api_key, args.email)
        print_success(f"Environment configuration created: {config_file}")
        
        # Step 12: Update bashrc
        print()
        print(f"{Colors.CYAN}{Colors.BOLD}[12/13]{Colors.END} {Colors.BLUE}Updating Bashrc{Colors.END}")
        update_bashrc(config_file)
        print_success("Bashrc updated with K2Think configuration")
        
        # Step 13: Create startup guide
        print()
        print(f"{Colors.CYAN}{Colors.BOLD}[13/13]{Colors.END} {Colors.BLUE}Creating Startup Guide{Colors.END}")
        guide_path = create_startup_guide(args.dir, api_key)
        print_success(f"Startup guide created: {guide_path}")
        
        # Print comprehensive setup summary
        print_setup_summary(args.dir, api_key, config_file, guide_path)
